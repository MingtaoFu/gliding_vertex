...
class PolygonInstance(object):
    ...
    def rotate90( self ):
        ...
        for p in self.polygons:
            ...
            new_p[ ::2] = p[1::2] # x = y
            new_p[1::2] = w - p[::2] # y = w - x
            rotated_polys.append( new_p )
        return PolygonInstance( rotated_polys, size=(h, w) )
...
class PolygonList(object):
    ...
    def rotate90( self ):
        """
        rboxes = torch.stack( list( map( lambda x: x.polygons[0], self.polygons ) ) )
        rotated_rboxes = torch.zeros_like( rboxes )
        rotated_rboxes[:,  ::2] = rboxes[:, 1::2] # x = y
        rotated_rboxes[:, 1::2] = w - rboxes[:, ::2] # y = w - x

        return PolygonList( rotated_rboxes[] )
        """

        rotated_polygons = []
        for polygon in self.polygons:
            rotated_polygons.append( polygon.rotate90() )

        return PolygonList( rotated_polygons, (self.size[1], self.size[0]) )
...
class SegmentationMask(object):
    ...
    # NOTE Mingtao
    def rotate90( self ):
        rotated_instances = self.instances.rotate90()
        return SegmentationMask( rotated_instances, (self.size[1], self.size[0]), self.mode )
