...
template <typename T>
__global__ void PSRoIAlignForward(const int nthreads, const T* bottom_data,
    ...
    CUDA_1D_KERNEL_LOOP(index, nthreads) {
        // NOTE Mingtao: copied from light-head
        // the order of dims is changed because the original implementation is in TensorFlow
        int n = index;
        ...
        T roi_width = fmaxf(roi_end_w - roi_start_w, 1.f);
        T roi_height = fmaxf(roi_end_h - roi_start_h, 1.f);
...
template <typename T>
__global__ void PSRoIAlignBackwardFeature(const int nthreads, const T* top_diff,
    ...
    CUDA_1D_KERNEL_LOOP(index, nthreads) {
        ...
        T* offset_bottom_diff =
            bottom_diff + (roi_batch_ind * channels + c) * height * width;//out_dim -> channels?

        int top_offset = (n * out_dim + ctop) * aligned_height * aligned_width; 
        const T* offset_top_diff = top_diff + top_offset;
        const T top_diff_this_bin = offset_top_diff[ph * aligned_width + pw];
...
at::Tensor PSROIAlign_forward_cuda(const at::Tensor& input,
  ...
  auto output = at::empty({num_rois, out_dim, pooled_height, pooled_width}, input.options());
  auto output_size = num_rois * pooled_height * pooled_width * out_dim;
  ...
  AT_DISPATCH_FLOATING_TYPES(input.type(), "PSROIAlign_forward", [&] {
...
at::Tensor PSROIAlign_backward_cuda(const at::Tensor& grad,
                                  const at::Tensor& rois,
                                  const float spatial_scale,
                                  const int pooled_height,
                                  const int pooled_width,
  ...
  AT_DISPATCH_FLOATING_TYPES(grad.type(), "PSROIAlign_backward", [&] {
    PSRoIAlignBackwardFeature<scalar_t><<<grid, block, 0, stream>>>(
         grad.numel(),
